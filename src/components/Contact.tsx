    ...
    <div className="bg-luxury-cream text-black min-h-screen">
      <section className="bg-[#0E0E0E] text-white pt-24 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-luxury-gold uppercase tracking-[0.22em] sm:tracking-[0.35em] text-[10px] mb-4 font-bold">Contact Hub</p>
          <h1 className="font-editorial text-4xl sm:text-5xl md:text-6xl font-light mb-5 leading-tight">Talk to sales or finance.</h1>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-10">
            ...
            {BRANCHES.map(branch => (
              <article key={branch.name} className="bg-white/70 border border-black/5 p-6">
                ...
                <div className="space-y-3 text-sm text-black/60">
                  <p className="flex items-start gap-3 break-words"><MapPin className="w-4 h-4 mt-0.5 shrink-0" /> {branch.location}</p>
                  <p className="flex items-start gap-3 break-words"><Phone className="w-4 h-4 mt-0.5 shrink-0" /> {branch.phone}</p>
                  <p className="flex items-start gap-3 break-all"><Mail className="w-4 h-4 mt-0.5 shrink-0" /> {branch.email}</p>
                  <p className="flex items-start gap-3 break-words"><Clock className="w-4 h-4 mt-0.5 shrink-0" /> {branch.hours}</p>
                </div>
              </article>
            ))}
          </div>
        ...
        <aside className="space-y-5">
          <a href={whatsappUrl('Hello Kiriro, I need assistance from sales or finance.')} target="_blank" rel="noreferrer" className="bg-green-700 text-white p-5 flex items-center justify-between gap-4 hover:bg-green-800 transition-colors">
